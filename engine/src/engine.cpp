...
void Engine::init(const std::string &title, size_t width, size_t height,
  ...
  if (!_window) {
    SDL_Quit();
    throw SDLException("window creation error: " + std::string(SDL_GetError()));
  }
...
void Engine::present() { SDL_RenderPresent(_renderer); }
...
size_t Engine::width() const { return _width; }

size_t Engine::height() const { return _height; }
